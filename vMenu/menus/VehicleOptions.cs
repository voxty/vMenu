        ...
        private void CreateMenu()
        {
            ...
            menu.OnItemSelect += (sender, item, index) =>
            {
                ...
                if (GetVehicle() != null && GetVehicle().Exists())
                {
                    ...
                    if (vehicle.GetPedOnSeat(VehicleSeat.Driver) == Game.PlayerPed)
                    {
                        ...
                        {
                            vehicle.Repair();
                            var logString = "Vehicle " + GetVehicleNumberPlateText(vehicle.Handle) + " repaired by " + Game.Player.Name;
                            TriggerServerEvent("vMenu:discordLogs", "vMenu: Vehicle Repair", logString, 3113019);
                        }
                        // Clean vehicle.
                        else if (item == cleanVehicle)
                        {
                            vehicle.Wash();
                            var logString = "Vehicle " + GetVehicleNumberPlateText(vehicle.Handle) + " cleaned " + Game.Player.Name;
                            TriggerServerEvent("vMenu:discordLogs", "vMenu: Vehicle Clean", logString, 3113019);
                        }
                        // Flip vehicle.
            ...
            menu.OnCheckboxChange += (sender, item, index, _checked) =>
            {
                ...
                else if (item == torqueEnabled) // Enable Torque Multiplier Toggled
                {
                    ...
                    var logString = "Vehicle " + GetVehicleNumberPlateText(vehicle.Handle) + $" torque {(VehicleTorqueMultiplier ? "toggled on" : "toggled off")} by " + Game.Player.Name;
                    TriggerServerEvent("vMenu:discordLogs", "vMenu: Vehicle Torque", logString, 3113019);
                }
                else if (item == powerEnabled) // Enable Power Multiplier Toggled
                {
                    VehiclePowerMultiplier = _checked;
                    var logString = "Vehicle " + GetVehicleNumberPlateText(vehicle.Handle) + $" power {(VehiclePowerMultiplier ? "toggled on" : "toggled off")} by " + Game.Player.Name;
                    TriggerServerEvent("vMenu:discordLogs", "vMenu: Vehicle Power", logString, 3113019);
                    if (_checked)
                    {
                        if (vehicle != null && vehicle.Exists())
                        {
